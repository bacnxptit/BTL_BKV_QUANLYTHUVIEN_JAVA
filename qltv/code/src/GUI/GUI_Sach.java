...
public class GUI_Sach extends JFrame {
	...
	DefaultTableModel Model = new DefaultTableModel();//thuộc tính để quản lí dữ liệu và cột trong bảng
	Vector<String> columns = new Vector<String>();
	Vector<Vector<Object>> rows = new Vector<>();
	Process_Sach ps = new Process_Sach();//lớp xử lí logic
	
	public void getAllSach() {
		Model.setRowCount(0);

		ArrayList<Sach> ls = ps.getListSach();//gọi phương thức getListSach của ps để hiện thị danh sách sách
		for (int i = 0; i < ls.size(); i++) {
			Sach s = (Sach) ls.get(i);
			Vector<Object> tbRow = new Vector<>();//tạo một vecto để chứa thông tin của sách
			tbRow.add(s.getMaSach());
			tbRow.add(s.getTenSach());
			tbRow.add(s.getTrangThai());	
			tbRow.add(s.getMaDauSach());
			rows.add(tbRow);
		}//duyệt từng đối tượng sách và thêm vào bảng bằng cách gọi phương thức getter từ đối tượng sách
		Model.setDataVector(rows, columns);//cập nhật dữ liệu cho cột va bảng
		table.setModel(Model);//gán model cho Jtable để hiện thị dữ liệu
	}
	...
	}//pt kiểm tra yêu cầu thêm sách thông qua đói tượng ps nếu thnhf công sẽ trả về true và lấy ds sách mới nhất cập nhật vào bảng
	public void updateSach(String MaSach, String TenSach, String TrangThai, String MaDauSach) {
		if (ps.updateSach(MaSach, TenSach, TrangThai, MaDauSach) == true) {
			...
			}//gọi pt updateSach cua doi tuong ps de cập nhật thông tin sách
			Model.setDataVector(rows, columns);
			table.setModel(Model);
		}
	}
	...
	}// phương thức lấy danh sach ma dau sách từ nguồn dữ liệu để thêm vào hộp chọn
	...
	}//phương thức lấy danh sách lấy tên sách từ nguồn dữ liệu để  thêm vào hộp chọn
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GUI_Sach frame = new GUI_Sach();//tạo lớp giao diện hiển thị sách kế thừa từ lớp jFrame
	...
	public GUI_Sach() {
		...
		setBounds(100, 100, 839, 733);//đặt ví trí và kích thước của cửa sổ
		contentPane = new JPanel();//tạo một Jpanenl chứa toàn bộ giao diện
		contentPane.setBackground(SystemColor.inactiveCaption);//đạt màu nền cho màu hệ thống màu xám nhạt
		contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));//đặt lề cho giao diện 5px cho mỗi cạnh
